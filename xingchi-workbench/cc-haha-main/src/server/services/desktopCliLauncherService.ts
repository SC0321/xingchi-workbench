...
const DESKTOP_CLI_NAME = 'xingchi-workbench'
const DESKTOP_CLI_WINDOWS_LEGACY_EXE = `${DESKTOP_CLI_NAME}.exe`
const PATH_BLOCK_START = '# >>> 星炽工作台 PATH >>>'
const PATH_BLOCK_END = '# <<< 星炽工作台 PATH <<<'
...
if [[ ! -x "$SIDECAR" ]]; then
  echo "xingchi-workbench launcher could not find bundled sidecar: $SIDECAR" >&2
  exit 127
fi
...
export function buildWindowsLauncherWrapper(sourcePath: string) {
  ...
  return [
    ...
    '  echo xingchi-workbench launcher could not find bundled sidecar: %SIDECAR% 1>&2',
    '  exit /b 127',
    ')',
