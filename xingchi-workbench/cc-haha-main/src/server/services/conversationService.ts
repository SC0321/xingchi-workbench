...
export class ConversationService {
  ...
  private resolveCliArgs(baseArgs: string[]): string[] {
    ...
    if (!launcher) {
      ...
      return [path.resolve(import.meta.dir, '../../../bin/xingchi-workbench'), ...baseArgs]
    }
  ...
  private buildStartupError(
    ...
    if (
      ...
      return new ConversationStartupError(
        'Desktop chat could not start because Claude CLI is not authenticated. Run `./bin/xingchi-workbench /login` or provide valid API credentials, then retry.',
        'CLI_AUTH_REQUIRED',
      )
    }
